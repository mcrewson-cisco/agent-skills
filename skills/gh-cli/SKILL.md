---
name: gh-cli
description: Use when working with GitHub from the terminal via the local gh CLI, especially for pull requests, issues, Actions, repositories, and auth checks.
---
...
In this Codex environment, prefer the GitHub plugin tools for standard PR, issue, and repository reads when they cover the task. Use `gh` when the user explicitly wants CLI behavior, when plugin coverage is missing, or when Actions/log inspection is easier from the terminal.

Run narrow `gh` read commands in the sandbox first. If a command fails with a likely sandbox or network restriction, rerun that same command with `sandbox_permissions: "require_escalated"` and a short justification tied to the user request.

When escalation is likely to be reused, include a narrow `prefix_rule` so the approval can persist. Prefer read-only prefixes such as:
...
If one of those approved command families answers the question, use it instead of a broader or adjacent command. For example, prefer `gh pr view 123 --json ...` over switching to `gh api`, and prefer `gh search prs ...` over a broader repository scan.

Do not request broad prefixes like `["gh"]` or mutation-capable prefixes like `["gh", "pr"]`. For write operations such as `gh pr create`, `gh pr review`, `gh pr merge`, `gh issue comment`, or `gh workflow run`, show the exact command and get user approval before running it.
